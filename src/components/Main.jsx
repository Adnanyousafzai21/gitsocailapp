...
const Main = ({ children }) => {
  ...
  return (
    <div className=' max-w-[900px]  flex flex-col bg-slate-200 py-2 gap-5 items-center md:items-start  md:flex-row m-auto  justify-center'>
      <div className={`md:w-[25%]  border w-[95%] hidden
      ${isDesktopOrTablet && location.pathname === "/profile" ? "hidden" : ""} sticky top-14  hidden`}>
        <Stories />
        </div>
      <div className='md:w-[40%]  w-[95%] border '>{children}</div>
      <div className='md:w-[25%] w-[95%]  hidden  sticky top-14'>
        <Features />
      </div>
    </div>
  )
