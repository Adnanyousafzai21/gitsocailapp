...
console.log(posts)
    ...
    return (
        <>
            <div className='w-full  bg-customwhite rounded-md '>
                <h1 className='text-center text-sky-500 my-2'>Your Posts</h1>
                {
                 posts  && posts.map((yourposts) => {
                        return <div className='w-full flex flex-col gap-2 mt-3 '>
                            ...
                            <p className={`py-2 px-3 ${!yourposts?.file? "bg-sky-300 w-full flex items-center justify-center rounded-md  py-2 text-center text-customwhite min-h-24 max-h-40 overflow-hidden":""}`}>{yourposts.description}</p>
                            <div className=''>
                                {yourposts?.file ? (
